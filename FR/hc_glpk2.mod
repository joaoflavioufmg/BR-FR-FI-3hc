...
# glpsol -m hc_glpk2.mod -d hc.dat -d pop_res.dat -d 7_dados.dat -d 7_dist_dur.txt
...
printf "%s\n", "----------------------------------------------------------------" >> "Resultado/00-Notes2.txt";
printf "%-5s\t%-6s\t%-25s:\t%-7s\n", "LEVEL", "CODE", "DESCRIPTION", "COST($)" >> "Resultado/00-Notes2.txt";printf{n in NA, j in J1: n=1 and INFR[j] = 1 and z[n,j] = 1}: "%5d\t%d\t%-25s:\t%.2f\n",1, j, Mun[j], CI[n]*HC[n]*POPN[n,j]*INFR[j]*z[n,j] >> "Resultado/00-Notes2.txt";
