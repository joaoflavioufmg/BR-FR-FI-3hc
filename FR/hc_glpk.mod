...
# printf "N\tOrigin\tDestination\tex\tpop\text/pop\t\tpop*ex\t\text\n";
# printf{n in NA, i in I,  j in J: ex[n,i,j] > 0 and pop[n,j] > 0}: "%d\t%d\t%d:\t\t%.2f\t%d\t%.2f\t\t%.2f\t\t%.2f\n", n,i,j, ex[n,i,j], pop[n,j], ext[n,i,j]/pop[n,j], pop[n,j]*ex[n,i,j], ext[n,i,j];
...
printf{n in NA:n=1}: "Care Level [%d]\t%-7d\t\t%.2f\n", n, POPmin[n], max{i in I , j in N1[i]}D[i,j]*x[n,i,j] >> "Resultado/00-Notes3.txt";
printf{n in NA:n=2}: "Care Level [%d]\t%-7d\t\t%.2f\n", n, POPmin[n], max{j in J1, k in N2[j]}D[j,k]*x[n,j,k] >> "Resultado/00-Notes3.txt";
printf{n in NA:n=3}: "Care Level [%d]\t%-7d\t\t%.2f\n", n, POPmin[n], max{k in J2, l in N3[k]}D[k,l]*x[n,k,l] >> "Resultado/00-Notes3.txt";
printf "%s\n", "----------------------------------------" >> "Resultado/00-Notes3.txt";
...
printf "%s\n", "----------------------------------------------------------------" >> "Resultado/00-Notes3.txt";
printf "%-5s\t%-6s\t%-25s:\t%-7s\n", "LEVEL", "CODE", "DESCRIPTION", "COST($)" >> "Resultado/00-Notes3.txt";
...
printf "%s\n", "----------------------------------------------------------------" >> "Resultado/00-Notes3.txt";
printf "%-5s\t%-6s\t%-25s:\t%-7s\n", "LEVEL", "CODE", "DESCRIPTION", "COST($)" >> "Resultado/00-Notes3.txt";
printf{n in NA, i in I : n=1 and sum{j in J1: i <> j} ex[n,i,j] > 0}: "%5d\t%d\t%-25s:\t%.2f\n",1, i, Mun[i], sum{j in N1[i]} f1[i,j]*P*D[i,j]*ext[n,i,j] >> "Resultado/00-Notes3.txt";
printf{n in NA, j in J1: n=2 and sum{k in J2: j <> k} ex[n,j,k] > 0}: "%5d\t%d\t%-25s:\t%.2f\n",2, j, Mun[j], sum{k in N2[j]} f2[j,k]*P*D[j,k]*ext[n,j,k] >> "Resultado/00-Notes3.txt";
printf{n in NA, k in J2: n=3 and sum{l in J3: k <> l} ex[n,k,l] > 0}: "%5d\t%d\t%-25s:\t%.2f\n",3, k, Mun[k], sum{l in N3[k]} f3[k,l]*P*D[k,l]*ext[n,k,l] >> "Resultado/00-Notes3.txt";
printf "%s\n", "----------------------------------------------------------------" >> "Resultado/00-Notes3.txt";


# # >>> Solution below is only available on hc_glpk1.mod
...
"---------------------------------------------";
# printf{n in NA}: "[%d]\t\t\t%-7d\t\t%.2f\n", n, POPmin[n], DurUpper[n] >> "Resultado/00-Notes3.txt";
printf{n in NA:n=1}: "Care Level [%d]\t%-7d\t\t%.2f\n", n, POPmin[n], max{i in I , j in N1[i]}D[i,j]*x[n,i,j];
printf{n in NA:n=2}: "Care Level [%d]\t%-7d\t\t%.2f\n", n, POPmin[n], max{j in J1, k in N2[j]}D[j,k]*x[n,j,k];
printf{n in NA:n=3}: "Care Level [%d]\t%-7d\t\t%.2f\n", n, POPmin[n], max{k in J2, l in N3[k]}D[k,l]*x[n,k,l];
printf "-------------------------------------------------------------------------------\n";
